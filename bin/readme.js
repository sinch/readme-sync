...
program.option(
  `-v, --${CONFIG_DOCSVERSION} <${CONFIG_DOCSVERSION}>`,
...
`
  ...
  .option(
    "-d, --dir <dir>",
    "Destination directory where docs Markdown files will be written",
    DEFAULT_DOCS_DIR
  )
  .action(async (slug, cmd) => {
    ...
    if (modifiedContentFiles.length > 0) {
      ...
      if (
        !readlineSync.keyInYN(
          "The above files have staged changes that could be overwritten. Are you sure you want to proceed?"
        )
      ) {
        return;
      }
    ...
    let fetchedPages = await readme.fetchPages(
      listCategories(options.categories, options.config),
      async (page) => {
        ...
        console.log(
          chalk.green(
            `Wrote contents of doc [${page.ref}] to file [${outputFile}]`
          )
        );
    ...
    const staleLocalPages = catalog.pages.filter(
      (page) => !fetchedPagePaths.includes(page.path)
    );

    if (staleLocalPages.length > 0) {
      console.log(
        chalk.yellow(
          `WARNING: Found ${staleLocalPages.length} possibly stale local content pages; they were not fetched after crawling Readme:`
        )
      );
      staleLocalPages.forEach((page) =>
        console.log(chalk.yellow(` - ${page.ref}`))
      );

      if (
        readlineSync.keyInYN(
          chalk.yellow(
            `They might have been deleted or moved. Do you want to prune these pages?`
          )
        )
      ) {
...
`
  ...
  .option(
    "-d, --dir <dir>",
    `Directory where the Markdown content files will be loaded from.`,
    DEFAULT_DOCS_DIR
  )
  .option(
    "-f, --file <file>",
    `Path to a single file to process, relative to the directory specified with -d/--dir option.`
  )
  .option(
    "--staged-only",
    `Only push files staged files that have been modified. Important: files must have been added to the index with 'git add'`
  )
  .option(
    "--prune",
    `When enabled, remote pages that do not exist locally will be pruned (deleted).`
  )
  .option(
    "--dry-run",
  ...
  .action(async (slug, cmd) => {
    ...
    if (options.prune) {
      ...
      let remotePages = await readme.fetchPages(
        listCategories(options.categories, options.config)
      );
...
`
  ...
  .option(
    "-w, --widgets <widgets>",
    `Comma-separated list of Readme widgets to replace to Markdown. Supported widgets: 'code', 'callout', 'image', 'html'`
  )
  .option(
    "-v, --verbose",
    `Output more details about the replacements being made.`
  )
  .option(
    "--no-download",
    `[For 'image' widgets only] Do not download remote image files to the local repository and replace URLs with relative paths to those files.`
  )
  .option(
    "--download-from <hostname>",
    `[For 'image' widgets only] Download images from specified host only.`,
    "files.readme.io"
  )
  .option(
    "--dry-run",
    `Will only output modifications that would be made, without actually saving them.`
  )
  .action(async (slug, cmd) => {
    const options = {
      config: loadConfigYaml(),
    ...
    for (const page of catalog.pages) {
      const updated = await markdownize.markdownize(
        page,
        options.widgets,
        options
      );
      if (!options.dryRun) {
        if (page.content !== updated) {
          ...
          console.log(
            chalk.green(`Writing updated Markdown to [${page.path}]`)
          );
...
`
  ...
  .option(
    ...
    `Comma-delimited list of validators to run. See command help for supported validators.`
  )
  .option(
    "--staged-only",
    `Only validate Git staged files. Important: files must have been added to the index with 'git add' for this to work.`
  )
  .option(
    "--no-fail",
    `By default, the command will exit with an error code if any validation errors are found. With this flag, the exit code will always be 0 (success).`
  )
  .action(async (slug, cmd) => {
    const options = {
      config: loadConfigYaml(),
      ...
      for (const validator of selectedValidators) {
        promises.push(
          validator.validate(entireCatalog, page, options, (element, err) => {
            console.log(
              `${chalk.cyan(element.ref)} [${chalk.yellow(
                element.desc
              )}]: ${err}`
            );
            errorCount++;
    ...
    Promise.all(promises).then(() => {
      if (errorCount > 0) {
        ...
        console.log(
          color(`${errorCount} validation errors were found in catalog.`)
        );
        if (options.fail) {
          process.exitCode = 1;
        }
...
function apiClient(catalog, options) {
  return new Api(
    globalOption(CONFIG_APIKEY),
    globalOption(CONFIG_DOCSVERSION),
    catalog,
    options
  );
}
...
async function selectPages(catalog, options) {
  ...
  if (options.file) {
    ...
    filters.push(
      Page.inCategories(listCategories(options.categories, options.config))
    );
  }
  ...
  if (options.stagedOnly) {
    ...
    filters.push((page) =>
      stagedFilePaths.includes(path.join(options.dir, page.path))
    );
  }
...
function globalOption(config, defaultValue) {
  ...
  if (value === undefined && defaultValue === undefined) {
    console.log(
      `Global option '${config}' is required. Provide it with --${config} option or ${envVar} environment variable.`
    );
    process.exit(1);
  }
  ...
  if (config.filters) {
    for (const [filterName, filterConfig] of Object.entries(config.filters)) {
      if (!(filterName in availableFilters)) {
        console.error(
          chalk.red(`Unknown filter [${filterName}] specified in config file.`)
        );
        process.exit(1);
      }
