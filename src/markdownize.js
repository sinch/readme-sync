...
async function replaceWidgets(page, content, widget, conversionFn, options) {
  const findAllBlocks = new RegExp(
    `\\[block:${widget}\\]\\n(?<json>(.|[\\r\\n])*?)\\n\\[/block\\]`,
    "gm"
  );
  ...
  return asyncStringReplace(content, findAllBlocks, async (match, json) => {
    ...
    if (options.verbose) {
      ...
      console.log(
        `${
          options.dryRun ? "Would be" : "Has been"
        } replaced by this Markdown: \n    `
      );
...
async function downloadImage(url, relativeToPage, options) {
  ...
  if (remoteUrl.host === options.downloadFrom) {
    ...
    if (options.dryRun) {
      console.log(
        chalk.cyan(
          `DRY RUN: Would download remote image [${url}] to local path [${localPath}]`
        )
      );
    } else {
      await new Promise((resolve, reject) => {
        ...
        request(url)
          .on("response", () => {
            console.log(
              chalk.green(
                `Downloaded remote image [${url}] to local path [${localPath}]`
              )
            );
            resolve();
          })
